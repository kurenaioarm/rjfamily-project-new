...
class LoginController extends \yii\web\Controller
{
    ...
    {
        //กรณ๊ติด process ให้เข้า SERVER เปิดสิทธิ์ sudo chmod -R -f 777 /var/www/html/web/RJMedicalRecord/assets
        $this->layout = '@app/../themes/login-template-1/views/layouts/login_RJMedicalRecord'; //กำหนด Template login_
...
//                        if ($rescall->success == true && $rescall->score >= 0.5){
                    ...
                    $PROJECT_ID = "007"; //เปลี่ยนตามรหัสโปรเจค
                    //===========================================

//                            if($chk_expdate == '2190100' || $chk_expdate == '3070100' ||   $chk_expdate == '2110000'){ //ส ิทธิ์การ Login

                    //===================== API RiskReport_Admin ========================
                    $API_medicalrecord_admin = $this->API_medicalrecord_admin($payload->staff_idcrd,"",$obj_access_token->access_token);
                    //================= เก็บ session แบบ Array ข้อมูลหลายชุด =======================
                    $arrayAccess_Token = array();
                    $arrayAccess_Token['admin'] = $API_medicalrecord_admin;
                    ...
                    return $this->redirect(['medicalrecord/index']);
//                            }else{
...
//                    var_dump($obj_access_token->json_result);
    ...
    public function API_medicalrecord_admin($ADMIN_ID,$ADMINTYPE_ID,$Token){
        $curl = curl_init();
